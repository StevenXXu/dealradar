...
def test_pipeline_scrapes_vc_portfolio(mock_fetch, tmp_path, monkeypatch):
    mock_fetch.return_value = (
        "<html><body><a href='https://canvas.co'>Canvas</a></body></html>"
    )
...
def test_faction_b_routes_to_jina_detail():
    ...
    json.dump(
        [
            {
                "name": "Investible",
                "url": "https://www.investible.com/portfolio",
                "slug": "investible",
                "faction_hint": "b",
            }
        ],
...
def test_scraper_warns_on_few_companies(capsys):
    ...
    if len(companies) < 3:
        print(
            f"  [WARN] {name} returned only {len(companies)} companies — below minimum threshold (3)"
        )
...
def test_scraper_tracks_vc_failure_rate(capsys):
    ...
    if total_vcs > 0 and failed_vcs > total_vcs / 2:
        print(
            f"  [CRITICAL] {failed_vcs}/{total_vcs} VCs returned 0 companies — pipeline may need attention"
        )
...
def test_pipeline_skips_completed_vcs(tmp_path, monkeypatch):
    ...
    state_file = tmp_path / "harvest_state.json"
    json.dump(
        {"completed_vcs": ["vc-a"], "last_updated": "2026-01-01T00:00:00Z"},
        state_file.open("w"),
    )
    monkeypatch.setattr("src.harvester.state.STATE_FILE", state_file)
    ...
    raw_file = tmp_path / "raw_companies.json"
    json.dump(
        [{"company_name": "Acme", "domain": "https://acme.co", "vc_source": "VC A"}],
        raw_file.open("w"),
    )
    ...
    pipeline = HarvesterPipeline(
        vc_seeds_path=str(seeds_file), output_path=str(raw_file)
    )
    pipeline.run()
